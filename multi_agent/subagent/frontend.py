...
from subagent.backend import stream



st.set_page_config(
    page_title="Movies Agent",
    layout="centered"
)


st.title("MoviesAgent")
...
with st.sidebar:
    ...
    thread_id = st.text_input("Active Thread ID", value="movies_session_1")
...
# User prompt input
if prompt := st.chat_input("Ask about movies, shows, viewing history, or ratings..."):
    with st.chat_message("user"):
        st.markdown(prompt)
